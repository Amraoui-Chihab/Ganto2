...
import '../main.dart';
class Companyloginphone extends StatefulWidget {
  const Companyloginphone({super.key});

  @override
  State<Companyloginphone> createState() => _CompanyloginphoneState();
}

class _CompanyloginphoneState extends State<Companyloginphone> {
                            ...
                            ElevatedButton(
                              onPressed: () async {
                                if (_key.currentState!.validate()) {
                                  ...
                                  await Future.delayed(Duration(milliseconds: 100)); // Give time to close the keyboard
                                await CompanyLoginWithPhone(_phoneNumber!,PasswordController.text);
                                }
                              },
  ...
  Future<void> CompanyLoginWithPhone(String Phone, String Password) async {
    try
    {
      Get.dialog(
      ...
      final url =
      Uri.parse('https://ganto-app.online/public/api/company/login/phone');

      var response = await http.post(url,
          body: {"company_phone": Phone, "company_password": Password});
    ...
    Get.back();
      ...
      Get.put(CompanyController(company),permanent: true);*/
        ...
        String companyJson = jsonEncode(company.toJson()); // Proper JSON encoding
